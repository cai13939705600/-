...
export default {
    ...
    methods: {
        ...
        handleExport(){
            ...
            return download({
                url: process.env.VUE_APP_API_PREFIX+'/ba/rest/api/storer/exportExcel',
                data: query
            });
        },
        // 编辑按钮响应事件
        ...
        // 启用
        handleStart (storeIds) {
            this.dispatch('startOrStop', { params: { idList: storeIds ,validFlag: 1}, cb: this.queryList })
        },
        // 停用
        handleStop (storeIds) {
            this.dispatch('startOrStop', { params: { idList: storeIds ,validFlag: 0}, cb: this.queryList })
        }
    }
}
</script>