...
export default {
    name: 'implement_instock_instockNotice',
    mixins: [ PageList ],
    components: {
        ExactSearch,
        ExactList,
    ...
    methods: {
        ...mapActions('stock/inStock', [
            ...
            //新增修改部分1
            ...mapMutations('implement/instock/instockNotice/stock', [ 'updateStockDetailList' ]),
            ...mapMutations('implement/instock/instockNotice/receive', ['updateReceivingDetailList' ]),
            ...mapMutations('implement/instock/instockNotice/putaway', [ 'updateWaitingPut' ]),
            ...mapMutations('implement/instock/instockNotice/deal', [ 'updateReceivingTrading' ]),
            //新增修改结束1
                ...
                this.handleEdit({
                    ...
                    body.asnDetailDtoList = body.asnDetailDtoList.map(item => {
                        return {
                            ...
                            grossWeight: item.skuWeight,
                            totalWeight:item.weight,
                            totalVolumn:item.volume,
                            volume: item.skuVolume,
                            remark: item.remark,
                            price: item.price,
        ...
        handleCommand(command) {
            if (command === 'import') {
                return this.proxyActions(command)
            }
            const rows= this.checkList;
            if(rows&&rows.length){
                const asnIdList = rows.map(row=>row.id);
