    ...
    actions: {
        ...generateActions({
            ...
            items: [
                ...
                // 根据出库单Id查找出库单明细列表 
                { key: 'getOutDetailListAction', callee: 'getDoInfoById' , mutation:'getOutDetailList'},
                // 修改
    ...
    mutations:{
        ...
        receivingGoodsDlgListDate: (state,receDlg) => {
            ...
            for (var i = 0; i < receDlg.length; i++) { 
                var myDate = new Date();
                receDlg[i].showAdd = true;
                //receDlg[i].toLocation = 'stage';
                receDlg[i].lotatt04 = jt.date.format(+new Date(myDate), 'yyyy-MM-dd')
                receDlg[i].receivedQty = receDlg[i].waitReceivedQty 
                //判断项目名称是否存在，存在则展示，否则不展示
                if( receDlg[i].itemId == 0) {
                    receDlg[i].itemId = ''
                }
                //给每条数据加一个隐藏id
                receDlg[i].id = i;
        ...
        //根据出库单id查找出库单明细列表
        getOutDetailList:(state,payload)=>{
            state.outDetailList = payload.doDetailList
        },
        //打印入库清单--数据
        printStock:(state,payload)=>{
            //state.printStockData={printAsnDetailList:payload}
