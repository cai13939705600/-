...
export default {
    ...
    data () {
        return {
            ...
            importResultMsg: '',
            importResultPath: '',
            tplUrl: process.env.VUE_APP_API_PREFIX + '/rest/api/skuBarcode/downloadExcelTemplate?' + qs.stringify({
        ...
        handleUpload (opts) {
            ...
            api.sku.importBarcodes(data).then(({ head, body }) => {
                if (head.code === '200' && body) {
                    ...
                    this.importResultPath = process.env.VUE_APP_API_PREFIX + '/rest/api/skuBarcode/downloadExcel?' + qs.stringify(params)
                }
